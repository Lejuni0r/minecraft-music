-- Pocket Music for CC:Tweaked
-- Streams DFPWM tracks from:
-- https://github.com/Lejuni0r/minecraft-music

local PLAYLIST_URL = "https://raw.githubusercontent.com/Lejuni0r/minecraft-music/main/playlist.json"
local PLAYLIST_CACHE = ".pocket_music_playlist.json"
local SETTINGS_FILE = ".pocket_music_settings.json"
...
if not speaker then
    error("Speaker introuvable. Equipe le Pocket Computer d'un Speaker Upgrade.", 0)
end
...
local function wake()
    os.queueEvent("pocket_music_wake")
end
...
local function cleanText(s)
    ...
    -- CC's terminal is not a general Unicode terminal. Keep titles readable.
    s = s:gsub("[\128-\255]", "")
    s = s:gsub("[%c]", " ")
    s = s:gsub("%s+", " ")
    s = s:gsub("^%s+", ""):gsub("%s+$", "")
...
local function getHeader(headers, wanted)
    ...
    for k, v in pairs(headers or {}) do
        if tostring(k):lower() == wanted then return v end
    end
    return nil
end
    ...
    for _, song in ipairs(list) do
        if type(song) == "table" then
            ...
            if url == "" and file ~= "" then
                url = "https://raw.githubusercontent.com/Lejuni0r/minecraft-music/main/songs/" .. file
            end
...
local function parsePlaylist(raw)
    if not raw or raw == "" then return nil, "Playlist vide" end
    ...
    if #list == 0 then return nil, "Aucun morceau valide" end
...
local function rebuildFilter(reset)
    ...
    if reset then
        ...
        state.selected = clamp(state.selected, 1, math.max(1, #state.filtered))
    end
end
...
local function fetchPlaylist()
    ...
    if oldFile then
        local remapped = nil
        for i, song in ipairs(state.playlist) do
            if song.file == oldFile then
                remapped = i
                break
            end
        end
        state.current = remapped
...
local function randomTrack()
    if #state.playlist <= 1 then return state.current or 1 end
...
local function previousTrack()
    ...
    if prevIndex < 1 then prevIndex = #state.playlist end
    setTrack(prevIndex)
end
...
local function setVolume(v)
    state.volume = clamp(math.floor(v * 4 + 0.5) / 4, 0, 3)
    saveSettings()
end

local function streamTrack(token)
    local song = state.playlist[state.current]
    if not song then return "cancelled" end
    ...
    while state.running
        ...
        if state.paused then
            os.pullEvent("pocket_music_wake")
        else
            ...
            while state.running
                and token == state.token
                and state.playRequested
                and not state.paused
                and not accepted do

                accepted = speaker.playAudio(audio, state.volume)
...
local function fillButton(x1, x2, y, label, active, action, value)
    ...
    writeAt(x1, y, text:sub(1, width), fg, bg)
    addHitbox(x1, y, x2, y, action, value)
end

local function currentProgress()
    local elapsed = state.bytesRead / BYTES_PER_SECOND
    local total = nil

    if state.totalBytes and state.totalBytes > 0 then
        total = state.totalBytes / BYTES_PER_SECOND
    elseif state.current and state.playlist[state.current]
        and state.playlist[state.current].duration then
        total = state.playlist[state.current].duration
    end
...
local function drawProgress(y)
    ...
    local ratio = 0
    if state.totalBytes and state.totalBytes > 0 then
        ratio = clamp(state.bytesRead / state.totalBytes, 0, 1)
    end
...
local function ensureSelectionVisible(rows)
    ...
    state.selected = clamp(state.selected, 1, #state.filtered)
    ...
    state.scroll = clamp(state.scroll, 0, math.max(0, #state.filtered - rows))
end

local function repeatLabel()
    if state.repeatMode == 0 then return "REP OFF" end
    if state.repeatMode == 2 then return "REP ONE" end
    return "REP ALL"
end
...
local function drawUI()
    ...
    writeAt(2, 1, "POCKET MUSIC", colors.white, colors.blue)

    local rightStatus
    if state.loading then
        rightStatus = "SYNC " .. spinnerFrames[spinnerIndex]
    ...
    writeAt(math.max(1, w - #rightStatus), 1, rightStatus, colors.white, colors.blue)

    -- Current title
    local song = state.current and state.playlist[state.current] or nil
    local title = songTitle(song)

    writeAt(1, 2, fit(title, w), state.error and colors.red or colors.white, colors.black)

    local elapsed, duration = currentProgress()
    local timeText = formatTime(elapsed) .. " / " .. formatTime(duration)
    writeAt(1, 3, center(timeText, w), colors.lightGray, colors.black)

    drawProgress(4)

    -- Previous / play / next
    local third = math.floor(w / 3)
    fillButton(1, third, 6, "<< PREV", false, "prev")
    fillButton(third + 1, w - third, 6, state.paused and "PLAY >" or "PAUSE", state.playRequested, "pause")
    fillButton(w - third + 1, w, 6, "NEXT >>", false, "next")

    -- Volume
    local volText = string.format("VOL %.2f", state.volume)
    local volThird = math.floor(w / 4)
    fillButton(1, volThird, 7, "-", false, "volDown")
    writeAt(volThird + 1, 7, center(volText, w - 2 * volThird), colors.yellow, colors.black)
    fillButton(w - volThird + 1, w, 7, "+", false, "volUp")

    -- Shuffle/repeat
    local half = math.floor(w / 2)
    fillButton(1, half, 8, state.shuffle and "SHUF ON" or "SHUF OFF", state.shuffle, "shuffle")
    fillButton(half + 1, w, 8, repeatLabel(), state.repeatMode ~= 0, "repeat")

    -- Search
    local searchLabel
    if state.searchMode then
        searchLabel = "SEARCH> " .. state.query .. "_"
    elseif state.query ~= "" then
        searchLabel = "SEARCH: " .. state.query
    else
        searchLabel = "SEARCH /"
    end
    clearLine(10, state.searchMode and colors.blue or colors.black)
    writeAt(1, 10, fit(searchLabel, w),
        state.searchMode and colors.white or colors.cyan,
        state.searchMode and colors.blue or colors.black)
    addHitbox(1, 10, w, 10, "search")

    -- Library
    local listStart = 11
    local listEnd = math.max(listStart, h - 2)
    local rows = listEnd - listStart + 1
    ensureSelectionVisible(rows)

    for row = 1, rows do
        local filteredPos = state.scroll + row
        local absIndex = state.filtered[filteredPos]
        local y = listStart + row - 1

        if absIndex then
            local item = state.playlist[absIndex]
            local isSelected = filteredPos == state.selected
            local isCurrent = absIndex == state.current

            local marker = isCurrent and ">" or " "
            local number = string.format("%03d", absIndex)
            local label = marker .. number .. " " .. songTitle(item)
            ...
            clearLine(y, bg)
            writeAt(1, y, fit(label, w), fg, bg)
            addHitbox(1, y, w, y, "track", filteredPos)
        else
    ...
    -- Footer
    local countText = string.format("%d/%d tracks",
        #state.filtered, #state.playlist)

    clearLine(h - 1, colors.black)
    writeAt(1, h - 1, fit(countText, w), colors.lightGray, colors.black)

    if state.error then
        clearLine(h, colors.red)
        writeAt(1, h, fit(state.error, w), colors.white, colors.red)
    else
        clearLine(h, colors.black)
        local hint = "SPACE Play  / Search  R Sync"
        writeAt(1, h, fit(hint, w), colors.gray, colors.black)
    end

    term.setCursorBlink(state.searchMode)
    if state.searchMode then
        local cursorX = math.min(w, 9 + #state.query)
        term.setCursorPos(cursorX, 10)
    end
end
...
local function action(name, value)
    ...
    elseif name == "track" then
        ...
        if pos and state.filtered[pos] then
            state.selected = pos
            setTrack(state.filtered[pos])
        end
    end
end
...
local function handleClick(x, y)
    for i = #hitboxes, 1, -1 do
        local b = hitboxes[i]
        if x >= b.x1 and x <= b.x2
            and y >= b.y1 and y <= b.y2 then
            action(b.action, b.value)
            return
        end
    end
end
...
local function handleNormalKey(key)
    ...
    elseif key == keys.enter then
        local absIndex = state.filtered[state.selected]
        if absIndex then setTrack(absIndex) end

    elseif key == keys.pageUp then
        state.selected = state.selected - 5
    ...
    elseif key == keys.backspace and state.query ~= "" then
        state.query = ""
        rebuildFilter(true)
...
local function handleSearchKey(key)
    ...
    elseif key == keys.backspace then
        if #state.query > 0 then
            state.query = state.query:sub(1, #state.query - 1)
            rebuildFilter(true)
        end
...
local function uiWorker()
    local timer = os.startTimer(UI_REFRESH)
    drawUI()
    ...
    while state.running do
        ...
        if ev == "timer" and a == timer then
            spinnerIndex = (spinnerIndex % #spinnerFrames) + 1
            drawUI()
            timer = os.startTimer(UI_REFRESH)
        ...
        elseif ev == "mouse_scroll" then
            local w, h = term.getSize()
            local rows = math.max(1, (h - 2) - 11 + 1)
            state.scroll = clamp(
                state.scroll + a,
                0,
...
local function splash(message)
    ...
    writeAt(1, math.max(1, math.floor(h / 2) - 1),
        center("POCKET MUSIC", w), colors.cyan, colors.black)

    writeAt(1, math.max(1, math.floor(h / 2) + 1),
        center(message or "Loading...", w), colors.lightGray, colors.black)
end

local function main()
    loadSettings()
    splash("Loading playlist...")

    if not fetchPlaylist() then
        splash("Playlist unavailable")
        sleep(1)
    end

    rebuildFilter(true)

    parallel.waitForAll(audioWorker, uiWorker)
